...
const OtherAudit: React.FC = () => {
  const { lang } = useLanguage();
  const content = pageContent.otherAudit[lang];
  ...
  return (
    <div className="min-h-screen">
      ...
      <PageHero
        ...
        backgroundImage="/images/other-services-hero.jpg"
        height="md"
        overlay="medium"
      />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <section className="mb-16">
            <h2 className="text-4xl font-bold text-cyber-emerald dark:text-white mb-8">
              {content.specializedServices.title}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {content.specializedServices.services.map((service, idx) => (
          ...
          <section className="p-8 bg-white dark:bg-gray-800 rounded-lg border border-cyber-emerald/20">
            <h2 className="text-3xl font-bold text-cyber-emerald dark:text-white mb-4">
              {content.whenToUse.title}
            </h2>
            <ul className="space-y-3">
              {content.whenToUse.occasions.map((occasion, idx) => (
                <li key={idx} className="text-gray-700 dark:text-gray-300 flex items-start gap-3">
                  <span className="text-cyber-emerald dark:text-digital-green">•</span>
                  <span>{occasion}</span>
                </li>
              ))}
            </ul>
